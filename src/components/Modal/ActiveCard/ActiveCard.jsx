 ...
 */
function ActiveCard() {
  // Không dùng biến state để check đóng mở modal nữa vì chúng ta sẽ check theo isShowModalActiveCard trong redux
  ...
  // fun dung chung cho các trường hợp update card title, description, cover, comment
  const callApiUpdateCard = async (updateData) => {
    const updatedCard = await updateCardDetailsAPI(activeCard._id, updateData)
    // b1 Cập nhật lại cái bản ghi card đang active trong modal
  ...
  const onUpdateCardTitle = (newTitle) => {
    if (newTitle.trim().length > 0 || newTitle.trim() === activeCard.title) {
  ...
  const onUpdateCardDescription = (newDescription) => {
    if (newDescription.trim().length > 0 || newDescription === activeCard.description) {
  ...
  // Dùng async await o day đe component con CardActivitySection chờ và nếu thành công thì mới clear thẻ input comment
    ...
    <Modal
      ...
      <Box sx={{
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          {/* Left side */}
          <Grid size={{ xs: 12, md: 12, sm: 9, xl: 9 }} >
            <Box sx={{ mb: 3 }}>
          ...
          {/* Right side */}
          <Grid size={{ xs: 12, md: 12, sm: 3, xl: 3 }}>
            <Typography sx={{ fontWeight: '600', color: 'primary.main', mb: 1 }}>Add To Card</Typography>
            <Stack direction="column" spacing={1}>
              {/* Feature 05: Xử lý hành động bản thân user tự join vào card */}
              {/* Nếu user hiện tại đang đăng nhập chưa thuộc mång memberIds của card thì mới cho hiện
              nut Join ra 
             Khi Click vào Join thì nó sẽ luôn là hành động ADD */}
              {!activeCard?.memberIds?.includes(currentUser._id) && <SidebarItem
